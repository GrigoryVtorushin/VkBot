...
async def q8_handler(message: Message):
    if message.text.lower() == answers[8].lower():
        ...
        return "ИскиН: Вот тебе ссылка на страницу моей цели. Ищи ключевые сл0ва или числа. Наверняка б0льшую часть таких слов ты найдёшь в подр0бной инф0рмации страницы польз0вателя. Но также ключевые сл0ва могут находиться в п0стах. Какое-нибудь важное мероприятие, чел0век, событие, партнер. Суть понял. Приступай."
...
async def q9_handler(message: Message):
    global k
    global tipsCopy
    if message.text in tipsCopy.keys():
        if k < 3:
            await message.answer("Да, это верн0. Прод0лжай искать, пока что мне не хватает инф0рмации.")
        else:
            await message.answer("Да, это верн0.")
        k += 1
        tipsCopy.pop(message.text)
    else:
        await message.answer("Нет, попробуй чт0-то другое.")
    if k == 4:
        await bot.state_dispenser.set(message.peer_id, Quest.Q10)
        if flag == 1:
            await message.answer("ИскиН: Есть. Совпадение хеша. Пар0ль подобран. Нак0нец-то…. Этот момент настал. По правде гов0ря, ты оказался очень полезным чел0веком. 0тправь мне результат, который у тебя получился, и наша ист0рия на этом завершится.", keyboard=choiseKeyboard1)
            return "Отправить правильный пароль?"
        else:
            await message.answer("ИскиН: Есть. Совпадение хеша. Пар0ль подобран. Нак0нец-то…. Этот момент настал. По правде гов0ря, ты оказался очень полезным чел0веком. 0тправь мне результат, который у тебя получился, и наша ист0рия на этом завершится. [--##НЕ ОТПРАВЛЯЙ ПАРОЛЬ##--]", keyboard=choiseKeyboard1)
            return "Отправить правильный пароль?"
...
async def q10_handler(message: Message):
    ...
    if message.text == "Нет":
        ...
        await message.answer("У тебя на руках остался правильный пароль и логин. Любопытство берет верх и ты решаешь войти самостоятельно. Это оказывается почта ректора ИРИТ-РТФ, Ильи Николаевича Обабкова. Ну, так тебе показалось на первый взгляд. На почте нет ничего, кроме одного видеофайла. Ты его открываешь.", attachment=attachment)
